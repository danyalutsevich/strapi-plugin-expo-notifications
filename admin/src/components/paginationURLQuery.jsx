...
const PaginationURLQuery = ({ pagination: { pageCount = 1 } }) => {
  ...
  const shouldShowDotsAfterFirstLink =
    pageCount > 5 ||
    (pageCount === 5 && (activePage === 1 || activePage === 5));
  const shouldShowMiddleDots =
    middleLinks.length > 2 && activePage > 4 && pageCount > 5;
...
PaginationURLQuery.propTypes = {
  pagination: PropTypes.shape({ pageCount: PropTypes.number.isRequired })
    .isRequired,
};
